...
def job_listing(request):
    ...
    context = {"query": qs, "job_qs": query, "nav": "job_listing"}
...
def job_post(request):
    ...
    context = {"form": form, "nav": "job_listing"}
...
def job_single(request, id):
    ...
    context = {"q": job_query, "nav": "job_listing"}
...
class SearchView(ListView):
    ...
    def get_queryset(self):
        ...
        for i in JobListing.objects.values_list():
            kieu_lam.append(i[4])
            loai_nghe.append(i[7])
            cho_lam.append(i[11])
            luong.append(int(i[12]))
        kieu_lam_response = self.count_list(kieu_lam)
        loai_nghe_response = self.count_list(loai_nghe)
        cho_lam_response = self.count_list(cho_lam)
        ...
        for x in set_data:
            for i in range(len(cho_lam)):
                if cho_lam[i] == x:
                    dict_data[x] += luong[i]
        cho_lam_luong_response = [list(dict_data.keys()), list(dict_data.values())]
...
def job_static(request):
    ...
    context = {
        ...
        "tong_nghe": tong_nghe_response,
        "tong_luong": tong_luong_response,
    }
